        ...
        <div className="max-w-4xl mx-auto px-4 py-12 text-on-surface">
            ...
            {error && !data ? (
                <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 mb-8">
                    ...
                    <p className="text-xs mt-1 opacity-80">Displaying last known cached statuses. Retrying automatically...</p>
                </div>
            ) : data ? (
                <div
                    className={`p-4 rounded-xl mb-8 flex items-center justify-between border ${data.overall === 'operational'
            ...
            <div className="space-y-4 mb-12">
                ...
                {data?.components.map((comp) => (
                    <div key={comp.id} className="p-5 rounded-xl border border-surface-border bg-surface-card shadow-sm">
                            ...
                            <div className="flex items-center gap-0.5 h-6">
                                {comp.uptime90Days.map((val, idx) => (
                                    <div
                                        ...
                                        className={`flex-1 h-full rounded-xs transition-colors hover:opacity-80 ${val === 1 ? 'bg-green-500/80' : val > 0 ? 'bg-amber-500' : 'bg-red-500'
                                            }`}
                                    />
                                ))}
                            </div>
                ...
                {data?.incidents && data.incidents.length > 0 ? (
                    <div className="space-y-6">
                        {data.incidents.map((incident) => (
                            <div key={incident.id} className="p-6 rounded-xl border border-surface-border bg-surface-card">
                    ...
                    <p className="text-sm text-on-surface-muted">No incidents reported in the recent period.</p>
                )}
            </div>
        </div>
    );
